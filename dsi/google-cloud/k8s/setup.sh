...
gcloud beta container clusters create sloan-r-workers \
  --zone us-central1-a \
  --num-nodes 20 \
  --disk-size 10GB \
  --machine-type n1-highmem-2 \
  --enable-autoscaling --min-nodes 2 --max-nodes 101 \
  --preemptible \
  --cluster-version=1.12.7 \
  --enable-stackdriver-kubernetes

# setup kubectl to connect to cluster
gcloud container clusters get-credentials sloan-r-workers --zone us-central1-a --project digital-ucdavis-edu
...
kubectl autoscale deployment sloan-r-worker-deployment --max 100 --min 1 --cpu-percent 60