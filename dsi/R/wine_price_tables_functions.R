...
nameBoxes <- function(data1, page.cols, prices = page.cols$prices, px , buffer = page.cols$charheight/2, charsize.cutoff = .4, psm = 3, text.level = "textline", min.words = 2, max.words = 8) { 
         ...
         # if the name now has some data (either originally or after expanded search), 
          if (tmp.name$char.sizes[1] !=0) {
            ...
            tmp.name2 = tmp.name %>% arrange(top) %>% 
                        ...
                        mutate(isPrice = !isPrice(text, maybe = T) %in% c("FALSE", "number", "ID"))
