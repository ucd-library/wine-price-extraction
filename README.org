...
#+RESULTS:
|   89 | entry_name_with_conf.csv    |
|  927 | entry_name_without_conf.csv |
| 1016 | total                       |
...
#+RESULTS:
: 850 name_summary_global_stats.csv
...
#+BEGIN_SRC sql
drop table if exists jane.price_name cascade;
...
with a as (
 ...
 from jane.crowd_source c join :s.entries e using (page_ark)
),
b as (
 select *,
