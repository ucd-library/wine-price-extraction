...
of this xwalk.  Part of the reason for that, is that with Eric's changes, we
have some new arks, from catalogs that were separated.
...
#+BEGIN_SRC bash
for i in $(find io/sloan-ocr/ -name \*_data1.RDS); do
 n=${i%_data1.RDS}; b=$(basename $n);
 docker exec -i $(sloan-dc ps -q r-base) R --slave --vanilla <<<"write.csv(readRDS('$i'),stdout())" |\
 sed -e "s/^/$b,/" |\ tee --append io/data1.csv ;
done
#+END_SRC
...
#+BEGIN_SRC sql
create table jane.overlapping_words as
select c.ark,
 c.word_id,
 j.jword_id,
 ...
 on (c.ark=j.ark and st_intersects(c.bbox,j.bbox) and
     ( st_area(st_intersection(c.bbox,j.bbox))/st_area(j.bbox) > 0.9 or
...
The first step is to run the parse_items.RDS command over all the data.  This creates
#+BEGIN_SRC bash
for i in $(find /io/sloan-ocr/D-* -name \*-[0-9][0-9][0-9].RDS); do
 d=$(dirname $i);
 echo $d;
 if [[ -f $d/parsed_items.RDS ]]; then
   echo $d/parsed_items.RDS exists;
 else
   Rscript --vanilla /opt/dsi/scripts/run_parse_items.R name.input.dir=$d name.output.dir=$d
  fi ;
done
#+END_SRC

#+BEGIN_SRC bash
for i in $(find /io/sloan-ocr/catalogs/ -name parse_folder_sample.RDS); do
 d=$(dirname $i);
 if [[ -f $d/PRICE_NAME.csv ]];
 then echo $d/PRICE_NAME.csv;
 else
  Rscript run_wine_database_one_page.R in=$i
 fi;
done
#+END_SRC

These can be combined to run at the same time.

#+BEGIN_SRC bash
s=/opt/dsi/scripts
R='Rscript --vanilla --quiet'
for i in $(find /io/sloan-ocr/D-* -name \*-[0-9][0-9][0-9].RDS | head -1 ); do
 d=$(dirname $i);
...
#+BEGIN_SRC bash
for i in $(find io/sloan-ocr/catalogs/ -name ENTRY_PAGE.csv); do
  f=$(basename $(dirname $i));
  cat $i | sed -e "s/NA$/$f/" |\
    psql service=sloan -c "\COPY jane.entry_page from STDIN CSV HEADER NULL 'NA'";
done
...
#+BEGIN_SRC sql
drop table if exists jane.entry_price;
...
Entry prices are collected together, where the page_ark is simply tacked on to
the end.

#+BEGIN_SRC bash
  rm entry_price.csv
  for i in $(find io/sloan-ocr/catalogs/ -name ENTRY_PRICE.csv); do
    f=$(basename $(dirname $i));
    tail -n +2 $i | sed -e "s/$/,$f/" >> entry_price.csv;
  done
  wc -l entry_price.csv
#+END_SRC

#+RESULTS:
: 33888 entry_price.csv
...
#+BEGIN_SRC bash
  rm entry_name.csv
  for i in $(find io/sloan-ocr/catalogs/ -name ENTRY_NAME.csv); do
    f=$(basename $(dirname $i));
...
#+RESULTS:
|   441 | entry_name_with_conf.csv    |
| 18345 | entry_name_without_conf.csv |
| 18786 | total                       |
...
#+BEGIN_SRC bash
  rm name_match.csv
  for i in $(find io/sloan-ocr/catalogs/ -name NAME_MATCH.csv); do
    f=$(basename $(dirname $i));
...
#+BEGIN_SRC bash
  ...
  for i in $(find io/sloan-ocr/catalogs/ -name name_summary_global_stats.csv); do
    f=$(basename $(dirname $i));
...
#+RESULTS:
: 19550 name_summary_global_stats.csv
...
#+BEGIN_SRC bash
  rm price_name.csv
  for i in $(find io/sloan-ocr/catalogs/ -name PRICE_NAME.csv); do
    f=$(basename $(dirname $i));
      tail -n +2 $i | sed -e 's/NULL/NA/g;' -e 's/"NA"/NA/g;' -e "s/$/,$f/" >> price_name.csv;
  done
  wc -l price_name.csv
#+END_SRC

#+RESULTS:
: 33888 price_name.csv
