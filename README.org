...
b as (
select name_id,
 array_agg("col.header" order by price_new::float) as price_header,
 array_agg(price_new::float order by price_new::float) as price,
 min("left") as l,
 max("right") as r,
 max(top) as t,
 min(bottom) as b
from :s.entry_price
where isnumeric(price_new)
group by name_id
),
c as (
select b.*,
...
with a as (
 select page_ark,
 split_part(page_ark,'-',1) as ark,
 split_part(page_ark,'-',2)::integer as page,
 name,
 vintage,
 'wine' as type,
 color,
